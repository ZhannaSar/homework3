...
In order to use knn technique on predicting the neighbors within NYC we need to load acs2017_ny file first.
```{r echo=TRUE}
...
The neighboring borough should share near similar characteristics. In this assignment we decide to use the cost of housing.
As we know a housing clover to the Central Business District (CBD) housings are smaller and more expensive to rent, while housings further away from CBD are larger - with more rooms, more affordable rents and higher utility cost due to size. Thus, we will consider overall cost of housing including rental and utility costs, and total household income (OWNCOST, RENT, COSTELEC, COSTFUEL, COSTGAS, COST WATER and INCOME).
```{r echo=TRUE}
